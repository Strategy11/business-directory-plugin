...
<div id="wpbdp-manage-listings-page" class="wpbdp-manage-listings-page businessdirectory-manage-listings businessdirectory wpbdp-page">
    <?php if ( ! $query->have_posts() ) : ?>
        <p><?php echo esc_html_x( 'You do not currently have any listings in the directory.', 'templates', 'WPBDM' ); ?></p>
        <?php
        echo sprintf(
            '<a href="%s">%s</a>.',
            esc_attr( wpbdp_get_page_link( 'main' ) ),
            esc_html_x( 'Return to directory', 'templates', 'WPBDM' )
        );
        ?>
    <?php else : ?>
        <p><?php echo esc_html_x( 'Your current listings are shown below. To edit a listing click the edit button. To delete a listing click the delete button.', 'templates', 'WPBDM' ); ?></p>
        <?php
