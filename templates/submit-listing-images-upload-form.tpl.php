...
<div class="image-upload-wrapper">
    ...
    <?php if ( is_admin() && ! wpbdp_is_request( 'ajax' ) ): ?>
        <div class="media-area-and-conditions cf">
            ...
            <p><?php _ex( 'or', 'templates image upload', 'business-directory-plugin' ); ?></p>
        </div>
    <?php endif; ?>
    ...
    <div id="image-upload-dnd-area" class="wpbdp-dnd-area <?php echo $admin ? 'no-conditions' : ''; ?>" data-action="<?php echo esc_url( wp_nonce_url( $action, 'listing-' . $listing_id . '-image-upload') ); ?>" data-admin-nonce="<?php echo $admin ? '1' : ''; ?>" >
            <div class="dnd-area-inside">
                <p class="dnd-message"><?php _ex( 'Drop files here', 'templates', 'business-directory-plugin' ); ?></p>
                <p><?php _ex( 'or', 'templates image upload', 'business-directory-plugin' ); ?></p>
                <p class="dnd-buttons"><label for="uploaded-images" class="upload-button"><a><?php _ex( 'Select images from your hard drive', 'templates', 'business-directory-plugin' ); ?></a><input id="uploaded-images" type="file" name="images[]" multiple="multiple" /></label></p>
            </div>
            <div class="dnd-area-inside-working" style="display: none;">
                <p><?php echo sprintf( _x( 'Uploading %s file(s)... Please wait.', 'templates', 'business-directory-plugin' ), '<span>0</span>' ); ?></p>
            </div>
