...
?>
    ...
    <?php if ( 'active' == $table->get_current_view() || 'all' == $table->get_current_view() ): ?>
        <div class="fees-order">
            ...
            <?php foreach ( array( 'asc' => _x( '↑ Ascending', 'fees admin', 'business-directory-plugin' ), 'desc' => _x( '↓ Descending', 'fees admin', 'business-directory-plugin' ) ) as $o => $l ): ?>
        ...
        <div class="gateway <?php echo $mod_info[0]; ?> <?php echo wpbdp_has_module( $mod_info[0] ) ? 'installed' : ''; ?>">
            ...
            <?php echo str_replace(
                ...
                sprintf( _x( 'You can buy the <a>%s</a> to add <a>%s</a> as a payment option for your users.',
                             'admin templates',
                             'business-directory-plugin' ), $mod_info[2], $mod_info[3] )
            ); ?>
            <?php endif; ?>
        </div>
        <?php endforeach; ?>
