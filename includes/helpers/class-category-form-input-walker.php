    ...
    public function start_el( &$output, $category, $depth = 0, $args = array(), $id = 0 ) {
        switch ( $this->input_type ) {
            case 'checkbox':
				...
				$output .= sprintf(
					'<label for="wpbdp-field-%7$d-%6$s"><input id="wpbdp-field-%7$d-%6$s" type="checkbox" class="%s" name="%s" value="%s" %s style="margin-left: %dpx;" /> %s</label>',
            ...
            default:
				$output .= sprintf(
					'<div class="wpbdm-form-field-radio-item"><label for="wpbdp-field-%6$d-%5$s"><input id="wpbdp-field-%6$d-%5$s" type="radio" name="%s" class="%s" value="%s" %s style="margin-left: %dpx;"> %s</label></div>',
					'listingfields[' . $this->field->get_id() . ']',
